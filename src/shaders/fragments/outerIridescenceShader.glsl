...
uniform sampler2D iridescenceMask; // Iridescence mask for dynamic visual effect

uniform float iridescenceIntensity; // Intensity of the iridescence effect
...
void main() {
    ...
    vec3 iridescenceEffect = iridescenceColor * fxMaskValue * smoothFactor * iridescenceIntensity * facingRatio;

    // Apply the iridescence effect to lighting
    vec3 finalLighting = iridescenceEffect * 10.0;

    // Add overall transparency to the effect
    float alphaValue = fxMaskValue * 0.3 * facingRatio; // Reduce opacity for both black and white parts, fades with facing ratio
